...
export default {
    ...
    actions: {
        authRequest() {                 //sends a request to back-end to renew authorization cookie. uses Google id_token to identify user.
            return new Promise ((resolve, reject) => {
                ...
                GoogleAuth.then(()=>{
                    ...
                    axios.get('/server.php', {params: {action: 'auth_request', id_token: idToken}}).then((response) => {
        ...
        uploadImage(context, payload) {    //request to upload image for 'owner' or 'dog' (specified in 'entity')
            return new Promise((resolve, reject) => {
                axios.post('/server.php', payload.image, {headers: {'ACTION' : 'upload_image', 'ENTITY' : payload.entity, 'ID': payload.id}}).then((response => {
        ...
        getMedicationInfo(context, dogId) {
            return new Promise ((resolve, reject) => {
                axios.get('/server.php', {params: {action: 'get_medication_info', dogId: dogId}}).then((response) =>{
        ...
        updateMedication(context, payload) {
            return new Promise((resolve, reject) => {
                axios.get('/server.php', {params: {action: 'update_medication', medication: payload}}).then((response) => {
                    if (response.data.msg ==='ok') {
                        //alert(response.data.msg);
                        resolve();
        ...
        updateUser(context, payload) {
            return new Promise ((resolve, reject) => {
                axios.get('/server.php', {params: {action: 'update_user', id: context.state.user.id, name: payload.name, country: payload.country, //<-- terrible practice, see updateDog
        ...
        getOwnerInfo(context, ownerId){   //LOAD owner AND his dogs (to owners, dogs)
            return new Promise((resolve, reject) => {
                axios.get('/server.php', {params: {action: 'get_owner_info', ownerId: ownerId}}).then((response) => {
        ...
        getOwnerDogs(context, ownerId) {      //!! MAY BE UNUSED   !!IS USED RIGHT BELOW!!!      //LOAD owner's dogs AND (if it is user) reload user's dogs //may be not that necessary but whatever
            return new Promise((resolve, reject) => {
                axios.get('/server.php', {params: {action: 'get_owner_dogs', ownerId: ownerId}}).then((response) => {
                    let msg = response.data.msg;
                    if (msg === 'ok') {
